...
def main() -> int:
    ...
    if manifest.get("release") != RELEASE or len(marks) != MARK_COUNT:
    ...
    for mark in marks:
        for key in ("id", "kind", "src", "label", "description"):
            if not mark.get(key):
    ...
    for marker in (
        'data-living-mark',
        'assets/iteration-19.css?v=0.19.0-public',
        'assets/iteration-19.js?v=0.19.0-public',
    ...
    for marker in ("[data-living-mark]", "prefers-reduced-motion", "object-fit: cover"):
