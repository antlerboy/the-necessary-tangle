...
p.write_text(s)
assert all(x in (target/'index.html').read_text() for x in ['vendor/leaflet.js','id="submission"','id="language"','name="languageRequirement"','Submit link','submissions.html','https://syscoi.com/','https://antlerboy.com/library/',canonical])
assert 'https://raw.githubusercontent.com/antlerboy/systemsmap/main/dist/' in (target/'app.js').read_text()
assert 'https://events.transduction.systems/api/submissions' in (target/'app.js').read_text()
assert 'window.location.assign' not in (target/'app.js').read_text()
assert (target/'submissions.html').exists() and (target/'submissions.js').exists()
print('Integrated systems events at /events/; collector, stable feeds, and submission queue remain in systemsmap.')

assert re.search(r"Quick submission\s*[—,;:-]\s*just a link", (target/"index.html").read_text())
