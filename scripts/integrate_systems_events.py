...
p.write_text(s)
assert all(x in (target/'index.html').read_text() for x in ['vendor/leaflet.js','id="submission"','id="language"','name="languageRequirement"','Quick submission — just a link','Submit link','submissions.html','https://syscoi.com/','https://antlerboy.com/library/',canonical])
